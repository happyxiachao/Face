...
import com.example.facerecognition.DlibFace.Dlibface;
...
public class Camera2BasicFragment extends Fragment
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    try {
      //初始化
      dlibface=new Dlibface(this.getContext());
   } catch (Exception e) {
  ...
  private void classifyFrame() {
    ...
    double rx=0.4,ry=0.3;
    ArrayList<Point> points= dlibface.RunFaceDet(bitmaps,rx,ry);
    if(points!=null) {
      for (int i = 0; i < points.size(); i++) {
        Point p = points.get(i);
        canvas.drawCircle((int)(p.x/rx), (int)(p.y/ry), 5, paint);
      }
    }
